<section class="login-page">
	<div class="form">
		<form class="login-form" action="login" method="post">
			<p class="register-form_title">
				Login in
			</p>
			<input type="text" name="login" placeholder="username" value="" required/>
			<input type="password" name="passwd" placeholder="password" value="" required/>
			<button type="submit" name="submit" value="OK" />login</button>
			<p class="message">Doesn't have an account? <a href="http://localhost:8100/user/signin">Sign In</a></p>
			<p class="message">If you forgot your password <a href="resetPass/initial">click here.</a></p>
  		</form>
	</div>
</section>